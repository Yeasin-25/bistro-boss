...
async function run() {
    try {
        ...
        app.post('/payments', verifyJWT, async (req, res) => {
            ...
            const quary = { _id: { $in: payment.cartItems.map(id => new ObjectId(id)) } };
        ...
        app.get('/admin-stats', verifyJWT, verifyAdmin, async (req, res) => {
            ...
            const revenue = payments.reduce((sum, payment) => sum + payment.price, 0).toFixed(0);
            res.send({
                users,
                products,
                orders,
...
app.listen(port, () => {
    console.log(`The Bistro Boss Runnig PORT: ${port}`)
})